    ...
    <div style={{ padding: '0 16px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 0 16px' }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-tertiary)', marginTop: 2 }}>Monitoring 8 Pakistan news sources</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={loadAlerts} style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', padding: 4 }}><RefreshCw size={18} /></button>
          <button onClick={() => navigate('/news-settings')} style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', padding: 4 }}><Settings size={20} /></button>
        </div>
      </div>
      ...
      {loading ? <LoadingSkeleton count={4} /> : alerts.length === 0 ? (
        <div className="card" style={{ padding: '40px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--text-tertiary)' }}>No alerts yet. RSS monitor is active.</div>
        </div>
      ) : (
              ...
              <div key={alert.id} style={{ background: pConfig.bg, border: `1px solid ${pConfig.borderColor}`, borderRadius: 16, padding: '16px', cursor: 'pointer' }} onClick={() => navigate(`/alerts/${alert.id}`)}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{alert.source_name}</span>
                    <span style={{ fontSize: 11, color: 'var(--text-disabled)' }}>{timeAgo(alert.detected_at)}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={e => { e.stopPropagation(); navigate(`/alerts/${alert.id}`) }} style={{ flex: 1, padding: '8px 12px', background: 'rgba(249,115,22,0.2)', border: '1px solid rgba(249,115,22,0.3)', color: '#fdba74', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>✏️ Edit & Post</button>
                  <button onClick={e => { e.stopPropagation(); navigate(`/alerts/${alert.id}`) }} style={{ flex: 1, padding: '8px 12px', background: 'rgba(249,115,22,0.8)', border: 'none', color: 'white', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>🚀 Post Now</button>
                  {alert.status === 'pending' && (
                    <button onClick={e => handleDismiss(alert.id, e)} style={{ padding: '8px 12px', background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', color: 'var(--text-tertiary)', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>✕</button>
                  )}
                </div>
              </div>
            )
