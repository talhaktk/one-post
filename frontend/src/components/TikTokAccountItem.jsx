    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
          <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>@{account.tiktok_username}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {onRemove && (
          <button onClick={() => onRemove(account.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-disabled)', padding: 4 }}>
            <Trash2 size={16} />
          </button>
        )}
      </div>
